package frc.robot; 

public class constants {
    public static int CARGO_INTAKE_DOWN = 0; 
    public static int CARGO_INTAKE_UP = 1;
    public static int CARGO_INTAKE_ACTUATOR = 2;
    public static int CARGO_INTAKE_ROLLER = 3;
    public static int HATCH_INTAKE_DOWN = 2; 
    public static int HATCH_INTAKE_UP = 3;
    public static int HATCH_INTAKE_ACTUATOR = 4; 
    public static int HATCH_INTAKE_ROLLER = 5; 
}