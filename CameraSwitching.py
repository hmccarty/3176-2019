...
def flipImage(frame):
    return cv2.flip(frame, -1) #rotate( frame, cv2.ROTATE_90_CLOCKWISE )
...
def findTargets(frame, mask, networkTable):
    # Finds contours
    ...
    if len(contours) != 0:
        image = findTape(contours, image, centerX, centerY, networkTable)
...
def findTape(contours, image, centerX, centerY, networkTable):
            ...
            if (checkContours(cntArea, hullArea)):
                ...
                for points in box:
                    ...
                    for coords in points:
                        #print("Block " + str(block) + " Point " + str(pidx) + " Corner " + str(cidx) + ": " + str(coords))
                        cidx += 1
                    pidx += 1
                if M["m00"] != 0:
                    cx = int(M["m10"] / M["m00"])
